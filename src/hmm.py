...
def generate_hmm_data(tagged_sentences):
    ...
    """
    words = list(
        set([word for tagged_sent in tagged_sentences for word, tag in tagged_sent]))
    words.sort()
    tags = list(
        set([tag for tagged_sent in tagged_sentences for word, tag in tagged_sent]))
    tags.sort()

    observation_dict = {word: i for i, word in enumerate(words)}
    state_dict = {state: i for i, state in enumerate(tags)}

    sentence_words = [[word for word, tag in tagged_sent]
                      for tagged_sent in tagged_sentences]
    sentence_tags = [[tag for word, tag in tagged_sent]
                     for tagged_sent in tagged_sentences]
...
class HMM:
    ...
    def fit(self, state_ids: List[List[int]], observation_ids: List[List[int]]):
        ...
        for s_ids, o_ids in zip(state_ids, observation_ids):
            ...
            for i in range(len(s_ids)):
                ...
                if i != len(s_ids) - 1:
                    self.A[s_ids[i], s_ids[i + 1]] += 1

                # count state->observations emissions
                self.B[s_ids[i], o_ids[i]] += 1
    ...
    def path_log_prob(self, state_ids: List[List[int]], observation_ids: List[List[int]]) -> np.array:
        ...
        for sent_states, sent_observations in zip(state_ids, observation_ids):
            ...
            observation_log_probs = np.array(
                [self.B[t, e] for t, e in zip(sent_states, sent_observations)])

            all_path_log_probs.append(
                transition_log_probs.sum() + observation_log_probs.sum())
    ...
    def decode(self, observation_ids: List[List[int]]) -> List[List[int]]:
        ...
        for obs_ids in observation_ids:
            ...
            # backpointers for each state+sequence id
            ...
            for t in range(1,T):
                for s in range(self.n):
                    potential_viterbis = viterbi[:, t-1] * self.A[:, s] *self.B[s, obs_ids[t]]
                    viterbi[s, t] = max(potential_viterbis)
                    back_pointer[s, t] = np.argmax(potential_viterbis)
            
            # follow the pointers to determine the best path
            # print(viterbi)
            # print(back_pointer)
  
            preds = []
            for stage in range(T, 0, -1):
                best_path_pointer = np.argmin(viterbi[:, stage - 1])
                preds.append(int(best_path_pointer))
...
def test_fit():
    ...
    train_states = [0]*len(observations1) + [1]*len(observations1) + \
        [2]*len(observations1) + [3]*len(observations1)
